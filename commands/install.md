...
```bash
# 설치된 캐시본을 우선 찾습니다. ~/.claude/plugins 전체를 한 번에 검색하면
# 마켓플레이스 체크아웃과 캐시본이 함께 잡히고, 둘의 순서가 비결정적이라
# head -1이 설치 버전과 다른 사본을 고를 수 있습니다.
src=$(find ~/.claude/plugins/cache -name "statusline-dots.sh" -path "*statusline-dots*" 2>/dev/null | head -1)
[ -z "$src" ] && src=$(find ~/.claude/plugins -name "statusline-dots.sh" -path "*statusline-dots*" 2>/dev/null | head -1)
echo "$src"
```

   결과가 비어 있으면 복사를 시도하지 말고 설치를 중단한 뒤, 플러그인이 정상 설치됐는지(`/plugin`) 확인하도록 사용자에게 안내하세요.
